...
echo git commit -m "$node_full_number" Dockerfile
echo git tag -d "$node_full_number" # This may fail if tag doesn't yet exist. That's OK
echo git push origin :refs/tags/$node_full_number && git tag -a $node_full_number -m "$node_full_number"
echo git tag -d "$node_major_number" # This may fail if such major ver doesn't yet exist. That's OK
echo git push origin :refs/tags/$node_major_number && git tag -a $node_major_number -m "$node_major_number"
